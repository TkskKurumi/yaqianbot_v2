...
def _has_tag(tag, to0, to1):
    ...
    return apply, revert
class IF(Block):
    def __init__(self, tag, then: Block, _else: Block=None):
        self.tail = Node()
...
def get_texts(ls, mx=2048, order=False):
    ...
    def recur(u: Node, max_branch):
        ...
        if(len(to)>max_branch):
            sample_n = max(1, ceil(max_branch))
            if (order):
                idx_to = list(enumerate(to))
                to = random.sample(idx_to, sample_n)
                to = [i for idx, i in sorted(to)]
            else:
                to = random.sample(to, sample_n)
        for v in to:
