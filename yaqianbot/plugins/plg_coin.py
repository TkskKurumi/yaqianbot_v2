...
ranking_dailyearly = jsondb(path.join(pth, "daily_ts"))
...
def cmd_mystatus(message: CQMessage):
    ...
    t_coin_num = Text("%.1f" % coins.get(
        user_id, 0), fontSize=coin_size, fill=dark_b.get_rgba(), bg=(0, 0, 0, 0))
    ...
    t_imp_label = Text("好感度", fontSize=coin_size,
                       fill=light_a.get_rgba(), bg=(0, 0, 0, 0))
    t_imp_num = Text("%.1f" % impression.get(user_id, 0),
    ...
    ret = Column([ret, t_hitokoto], borderWidth = border, bg=bgc)
    ret = CompositeBG(ret, bg=bg)
    message.response_sync(ret.render())
